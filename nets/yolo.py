...
#---------------------------------------------------#
#   使用yolov8官方颈部网络，将dct特征融合入主干
#---------------------------------------------------#
class YoloBody_dct(nn.Module):
    def __init__(self, input_shape, num_classes, phi, pretrained=False):
        super(YoloBody_dct, self).__init__()
        depth_dict          = {'n' : 0.33, 's' : 0.33, 'm' : 0.67, 'l' : 1.00, 'x' : 1.00,}
        width_dict          = {'n' : 0.25, 's' : 0.50, 'm' : 0.75, 'l' : 1.00, 'x' : 1.25,}
        deep_width_dict     = {'n' : 1.00, 's' : 1.00, 'm' : 0.75, 'l' : 0.50, 'x' : 0.50,}
        dep_mul, wid_mul, deep_mul = depth_dict[phi], width_dict[phi], deep_width_dict[phi]
    ...
    def forward(self, x, dct):
        #  backbone
        feat1, feat2, feat3 = self.backbone.forward(x, dct)
...
#-------------------------------------------------#
#   使用yolov8官方颈部网络，将dct特征融合入颈部中
#-------------------------------------------------#
class YoloBody_dct2(nn.Module):
    def __init__(self, input_shape, num_classes, phi, pretrained=False):
        super(YoloBody_dct2, self).__init__()
        depth_dict          = {'n' : 0.33, 's' : 0.33, 'm' : 0.67, 'l' : 1.00, 'x' : 1.00,}
        width_dict          = {'n' : 0.25, 's' : 0.50, 'm' : 0.75, 'l' : 1.00, 'x' : 1.25,}
        deep_width_dict     = {'n' : 1.00, 's' : 1.00, 'm' : 0.75, 'l' : 0.50, 'x' : 0.50,}
        dep_mul, wid_mul, deep_mul = depth_dict[phi], width_dict[phi], deep_width_dict[phi]
        ...
        #---------------------------------------------------#
        # self.backbone   = Backbone_RGB_DCT_fusion1(base_channels, base_depth, deep_mul, phi, pretrained=pretrained)
        self.backbone   = Backbone_RGB_DCT_fusion6(base_channels, base_depth, deep_mul, phi, pretrained=pretrained)
        
        #------------------------加强特征提取网络------------------------# 
        self.upsample   = nn.Upsample(scale_factor=2, mode="nearest")

        # 1024 * deep_mul + 512, 40, 40 => 512, 40, 40
        self.conv3_for_upsample1    = C2f(int(base_channels * 16 * deep_mul) + base_channels * 8 + 96 + 128, base_channels * 8, base_depth, shortcut=False)
        # 768, 80, 80 => 256, 80, 80
        self.conv3_for_upsample2    = C2f(base_channels * 8 + base_channels * 4 + 64, base_channels * 4, base_depth, shortcut=False)
        ...
        self.down_sample2           = Conv(base_channels * 8, base_channels * 8, 3, 2)
        # 1024 * deep_mul + 512, 20, 20 =>  1024 * deep_mul, 20, 20
        self.conv3_for_downsample2  = C2f(int(base_channels * 16 * deep_mul) + base_channels * 8 + 128, int(base_channels * 16 * deep_mul), base_depth, shortcut=False)
        #------------------------加强特征提取网络------------------------# 
...
#   只修改颈部网络，添加颈部网络
#---------------------------------------------------#
class YoloBody_neck(nn.Module):
    def __init__(self, input_shape, num_classes, phi, pretrained=False):
        super(YoloBody_neck, self).__init__()
        depth_dict          = {'n' : 0.33, 's' : 0.33, 'm' : 0.67, 'l' : 1.00, 'x' : 1.00,}
        width_dict          = {'n' : 0.25, 's' : 0.50, 'm' : 0.75, 'l' : 1.00, 'x' : 1.25,}
        deep_width_dict     = {'n' : 1.00, 's' : 1.00, 'm' : 0.75, 'l' : 0.50, 'x' : 0.50,}
        dep_mul, wid_mul, deep_mul = depth_dict[phi], width_dict[phi], deep_width_dict[phi]
...
#---------------------------------------------------#
#   修改DCT和颈部网络，添加dct特征融合模块和颈部网络，将特征融合入主干当中
#---------------------------------------------------#
class YoloBody_dct_neck(nn.Module):
    def __init__(self, input_shape, num_classes, phi, pretrained=False):
        super(YoloBody_dct_neck, self).__init__()
        depth_dict          = {'n' : 0.33, 's' : 0.33, 'm' : 0.67, 'l' : 1.00, 'x' : 1.00,}
        width_dict          = {'n' : 0.25, 's' : 0.50, 'm' : 0.75, 'l' : 1.00, 'x' : 1.25,}
        deep_width_dict     = {'n' : 1.00, 's' : 1.00, 'm' : 0.75, 'l' : 0.50, 'x' : 0.50,}
        dep_mul, wid_mul, deep_mul = depth_dict[phi], width_dict[phi], deep_width_dict[phi]
        ...
        #-------------------------------------------------#
        self.neck = GiraffeNeckV2(depth = 0.5,
                                  hidden_ratio = 0.5,
...
#---------------------------------------------------#
#   修改DCT和颈部网络，添加dct特征融合模块和颈部网络，将特征融合入颈部当中
#---------------------------------------------------#
class YoloBody_dct_neck2(nn.Module):
    def __init__(self, input_shape, num_classes, phi, pretrained=False):
        super(YoloBody_dct_neck2, self).__init__()
        depth_dict          = {'n' : 0.33, 's' : 0.33, 'm' : 0.67, 'l' : 1.00, 'x' : 1.00,}
        width_dict          = {'n' : 0.25, 's' : 0.50, 'm' : 0.75, 'l' : 1.00, 'x' : 1.25,}
        deep_width_dict     = {'n' : 1.00, 's' : 1.00, 'm' : 0.75, 'l' : 0.50, 'x' : 0.50,}
        dep_mul, wid_mul, deep_mul = depth_dict[phi], width_dict[phi], deep_width_dict[phi]
        ...
        self.neck = GiraffeNeckV2(depth = 0.5,
                                  hidden_ratio = 0.5,
                                  in_channels = [64+64, 128+96, 256+128],
                                  out_channels = [64, 128, 256],
                                  block_name = "BasicBlock_3x3_Reverse",
                                  depthwise = True)
